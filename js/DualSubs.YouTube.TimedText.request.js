		...
		switch ($response) {
			...
			case undefined: // 无构造回复数据，发送修改的请求数据
				...
				switch (Format) {
					...
					case "application/grpc":
						// 返回二进制数据
						if ($.isQuanX()) $.done({ headers: $request.headers, bodyBytes: $request.bodyBytes.buffer.slice($request.bodyBytes.byteOffset, $request.bodyBytes.byteLength + $request.bodyBytes.byteOffset), opts: $request.opts });
						else $.done($request);
						break;
					case undefined: // 视为无body
						// 返回普通数据
