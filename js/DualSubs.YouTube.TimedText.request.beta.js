...
const $ = new Env("🍿 DualSubs: ▶ YouTube v0.8.0(13) timedtext.request.beta");
const URL = new URLs();
const DataBase = {
	"Default": {
						...
						case "m.youtube.com":
							...
							switch (PATH) {
								case "api/timedtext":
									// 格式化缓存
									if (url?.query?.tlang) {
										Caches.tlang = url.query.tlang; // 保存目标语言
										$.setdata(Caches.tlang, `@DualSubs.${"YouTube"}.Caches.tlang`);
									};
									if (url?.query?.v && url?.query?.lang && !url?.query?.tlang) {
										Caches.Playlists.Subtitle.set(url.query.v, url.query.lang); // 保存原文语言
