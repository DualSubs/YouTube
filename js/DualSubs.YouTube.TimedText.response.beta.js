...
const $ = new Env("🍿 DualSubs: ▶ YouTube v0.8.0(9) timedtext.response.beta");
const URL = new URLs();
const XML = new XMLs();
...
const DataBase = {
	...
	"YouTube": {
		"Settings": {"Switch":true,"Type":"Official","Language":"AUTO"},
		"Configs": {
...
(async () => {
	...
	switch (Settings.Switch) {
		...
		default:
			...
			// 设置自定义参数
			const Type = url?.query?.subtype || url?.query?.dualsubs || Settings.Type, Languages = url?.query?.sublang || Settings.Languages;			$.log(`🚧 ${$.name}, Type: ${Type}, Languages: ${Languages}`, "");
			...
			switch (Format || FORMAT) {
				...
				case "application/x-plist":
					...
					for await (let request of requests) {
						...
						body = CombineDualSubs(OriginSub, SecondSub, Format || FORMAT, Kind, Settings.Offset, Settings.Tolerance, [Settings.Position]);
					};
					$.log(`🚧 ${$.name}`, `OriginSub: ${JSON.stringify(OriginSub)}`, "");
