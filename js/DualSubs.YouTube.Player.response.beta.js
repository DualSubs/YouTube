...
const $ = new Env("🍿️ DualSubs v0.6.2(3)-youtube-player-beta");
const URL = new URLs();
const DataBase = {
	"Verify": {
					...
					if (data?.captions) { // 有基础字幕
						...
						if (data?.captions?.playerCaptionsTracklistRenderer) { // 有轨道列表
							...
							if (data?.captions?.playerCaptionsTracklistRenderer?.audioTracks) {
								data.captions.playerCaptionsTracklistRenderer.audioTracks.visibility = "ON";
								data.captions.playerCaptionsTracklistRenderer.audioTracks.captionsInitialState = "CAPTIONS_INITIAL_STATE_ON_RECOMMENDED";
